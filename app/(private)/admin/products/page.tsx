...
import { Search, Plus, Eye, Edit, Palette, Trash2, Package, DollarSign, User, LayoutTemplate } from "lucide-react"
...
export default function Page() {
  ...
  const [productTemplates, setProductTemplates] = React.useState<Record<string, TemplateSummaryItem[]>>({})
      ...
      try {
        ...
        if (!ignore) {
          ...
          for (const product of res.data?.items ?? []) {
            try {
              const templateRes = await getTemplatesByProduct(product.productId)
              if (templateRes.success) {
                templates[product.productId] = templateRes.data ?? []
              }
            } catch (error) {
              console.error(`Failed to load templates for product ${product.productId}:`, error)
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-muted-foreground">Quản lý và theo dõi tất cả sản phẩm trong hệ thống</p>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">Sản phẩm hiện có</p>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {items.length > 0 
                      ? formatCurrency(
                          items.reduce((sum, item) => sum + (item.minPrice + item.maxPrice) / 2, 0) / items.length,
                          'VND', 'vi-VN'
                        )
                      : '0 ₫'
                    }
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">Hệ thống hoạt động</p>
                </CardContent>
              </Card>
            </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    {!isLoading && items.map((p) => (
                      <TableRow key={p.productId} className="hover:bg-muted/50 transition-colors">
                        <TableCell>
                          <div className="space-y-1">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <span>{p.createdBy?.name || 'Không xác định'}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="outline" size="sm" asChild className="h-8">
                              <Link href={`/admin/products/${p.productId}`} className="flex items-center gap-1">
                                <Eye className="h-3 w-3" />
                                Xem
                              </Link>
                            </Button>
                            <Button variant="secondary" size="sm" asChild className="h-8">
                              <Link href={`/admin/products/${p.productId}/edit`} className="flex items-center gap-1">
                                <Edit className="h-3 w-3" />
                                Sửa
                              </Link>
                            </Button>
                            <Button variant="default" size="sm" asChild className="h-8">
                              <Link href={getTemplateUrl(p.productId)} className="flex items-center gap-1">
                                <Palette className="h-3 w-3" />
                                {(productTemplates[p.productId] || []).length > 0 ? 'Sửa mẫu' : 'Tạo mẫu'}
                              </Link>
                            </Button>
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Hành động này không thể hoàn tác. Sản phẩm <strong>"{p.name}"</strong> sẽ bị xóa vĩnh viễn khỏi hệ thống.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Hiển thị {items.length} sản phẩm trên trang {currentPage} / {totalPages}
                </div>
                <Pagination>
                  <PaginationContent>
                    <PaginationItem>
